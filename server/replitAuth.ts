...
import { seedSuperAdmin } from "./roleAuth";
...
/**
 * Dual authentication middleware that supports both OIDC and password sessions
 */
export const isAuthenticatedAny: RequestHandler = async (req, res, next) => {
  ...
  // Handle password-based authentication (no token expiry check needed)
  if (user.authMethod === 'password') {
    return next();
  }

  // Handle OIDC authentication (existing logic)
  if (!user.expires_at) {
    return res.status(401).json({ message: "Unauthorized" });
  }
